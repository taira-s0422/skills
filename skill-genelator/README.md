...
**しょーぱん（GITEC 生成AIコンサルタント）が自作した Claude Code スキル & サブエージェント コレクション**

実務で鍛えた11のスキルと、各スキルに特化した12体の個性的なサブエージェントを収録。ブランディングからCMS構築、提案書生成まで、AIコンサルタントの業務をカバーする。
...
```bash
# 例: brand スキルをインストール
cp -r skill-genelator/.claude/skills/brand ~/.claude/skills/
```
...
cp skill-genelator/.claude/agents/skill-master.md ~/.claude/agents/
```
